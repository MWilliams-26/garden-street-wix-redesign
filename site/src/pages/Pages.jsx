...
const explore = [
  ...
  ['Camps & Performances','/camps-performances','campWide'], ['Parties & Rentals','/parties-rentals','studioMain'],
];
const agePaths = [
...
export function Home() { return <><Seo page="home" path="/" />
  ...
  <section id="ages" className="home-ages"><Section title="Find the right starting point" eyebrow="Start with age"/><div className="age-row">{agePaths.map(([age, description])=><Link to={`/classes?age=${encodeURIComponent(age)}`} key={age}><strong>{age}</strong><span>{description}</span></Link>)}</div></section>
  <section className="tint home-programs"><Section title="Choose what works for your family." eyebrow="Programs & services"/><div className="visual-cards explore-grid">{explore.map(([title,to,image])=><Link to={to} className="visual-card" key={title}><ResponsiveImage image={images[image]}/><h3>{title}<span aria-hidden="true">→</span></h3></Link>)}</div></section>
...
export function Classes() {
 ...
 <section className="class-finder"><Section eyebrow="Find a class" title="Start with your child’s age."/><p className="finder-intro">Choose an age group, then narrow the results by interest or day if helpful.</p>
  <fieldset className="choice-group"><legend>Child’s age</legend><div className="age-choices">{classAgeGroups.map(([label])=><button type="button" className={age===label?'choice active':'choice'} aria-pressed={age===label} onClick={()=>{setDay('Any day');setSearchParams({age:label},{replace:true})}} key={label}>{label}</button>)}</div></fieldset>
...
export function Parties(){return <><Seo page="parties-rentals" path="/parties-rentals"/><Hero image={images.studioMain} title="Make their celebration move." text="Host an energetic performing-arts party or rent studio space in Hoboken."><a className="button" href={`mailto:${siteSettings.email}?subject=Party%20inquiry`}>Plan a party</a></Hero><section className="parties-page"><div className="cards package-grid">{packages.map(x=><article className="card" key={x[0]}><h2>{x[0]}</h2><p className="price">{x[1].split(' / ').map((part,i)=><span key={part}>{part}{i===0&&<span aria-hidden="true"> / </span>}</span>)}</p><p>{x[2]}</p></article>)}</div><div className="party-details"><p><strong>An 18% gratuity applies to all party packages.</strong></p><p>DIY rentals include exclusive use of the studio. Renters bring their own food, drinks, cake, decorations and entertainment and are responsible for cleanup and removing all supplies and trash.</p><h2>Choose a theme</h2><div className="tags">{['Ballerina Princess','K-Pop','TikTok Dance Lab','Dance Class','Creative Music & Movement'].map(x=><span key={x}>{x}</span>)}</div></div><Gallery className="party-gallery" names={['studioMain','studioB','brandedWall']}/></section></>}
export function About(){return <><Seo page="about" path="/about"/><Hero className="about-hero" image={images.owners} title="A welcoming performing arts community since 2005." text="Garden Street helps children train with purpose, perform with confidence and grow as artists and individuals."/><section className="split about-mission"><Section eyebrow="Our mission" title="Train with purpose. Perform with confidence. Grow together."/><p>Our mission is to provide exceptional performing arts education in a welcoming and supportive environment where every child has the opportunity to train with purpose, perform with confidence, and grow as an artist and individual.</p></section><section className="split tint about-story"><Section eyebrow="Our story" title="A new chapter, rooted in Hoboken."/><div><p>Founded in 2005, Garden Street School of the Performing Arts has inspired children to discover their creativity, build confidence and develop a lifelong love of the performing arts.</p><p>In June 2026, the studio entered a new chapter under the ownership of Victoria and Cory Johnson, continuing its commitment to performing-arts education in a warm, welcoming community.</p></div></section><section className="tint about-team"><Section eyebrow="Our team" title="The people who make the studio feel like home."/><p className="team-intro">Led by owner and director Victoria Johnson, our team gives children a supportive place to learn, create and belong.</p><div className="cards two">{staff.map(x=><article className="card" key={x.name}><ResponsiveImage image={images[x.image]}/><div className="team-card-copy"><h3>{x.name}</h3><p className="team-role">{x.role}</p><p>{x.bio}</p></div></article>)}</div></section></>}
export function Contact(){return <><Seo page="contact" path="/contact"/><Hero className="contact-hero" image={images.exterior} title="Let’s connect." text="Questions about finding the right program? Reach the Garden Street team directly."/><section className="split contact-section"><ResponsiveImage className="contact-image" image={images.brandedWall}/><div className="contact-details"><p className="eyebrow">Visit us</p><h2>Find us in the heart of Hoboken.</h2><address>{siteSettings.address}</address><p><a href={`tel:${siteSettings.phone.replaceAll('-','')}`}>{siteSettings.phone}</a><br/><a href={`mailto:${siteSettings.email}`}>{siteSettings.email}</a></p><div className="contact-actions"><ExternalCta href={externalLinks.directions}>Get directions</ExternalCta><ExternalCta href={externalLinks.generalRegistration}>Registration</ExternalCta></div><div className="contact-secondary"><a href={`mailto:${siteSettings.email}?subject=Party%20inquiry`}>Party inquiry</a><ExternalCta className="text-link" href={externalLinks.instagram}>Instagram</ExternalCta></div></div></section></>}
