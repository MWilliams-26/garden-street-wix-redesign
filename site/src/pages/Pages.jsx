...
export function Classes() {
 ...
 <section className="class-finder"><Section eyebrow="Find a class" title="Start with your child’s age."/><p className="finder-intro">Choose an age group, then narrow the results by interest and the days that work for your family.</p>
...
export function About(){return <><Seo page="about" path="/about"/><Hero className="about-hero" image={images.owners} title="A welcoming performing arts community since 2005." text="Garden Street helps children train with purpose, perform with confidence and grow as artists and individuals."/><section className="split about-mission"><Section eyebrow="Our mission" title="Train with purpose. Perform with confidence. Grow together."/><p>Our mission is to provide exceptional performing arts education in a welcoming and supportive environment where every child has the opportunity to train with purpose, perform with confidence, and grow as an artist and individual.</p></section><section className="split tint about-story"><Section eyebrow="Our story" title="A new chapter, rooted in Hoboken."/><div><p>Founded in 2005, Garden Street School of the Performing Arts has inspired children to discover their creativity, build confidence and develop a lifelong love of the performing arts.</p><p>In June 2026, the studio entered a new chapter under the ownership of Victoria and Cory Johnson, continuing its commitment to performing-arts education in a warm, welcoming community.</p></div></section><section className="tint about-team"><Section eyebrow="Our team" title="The people who make the studio feel like home."/><p className="team-intro">Led by owner and director Victoria Johnson, our team gives children a supportive place to learn, create and belong.</p><div className="cards two">{staff.map(x=><article className={`card${x.detailsPending?' team-card-pending':''}`} key={x.name}><ResponsiveImage image={images[x.image]}/><div className="team-card-copy"><h3>{x.name}</h3>{x.detailsPending?<p className="team-pending">Role and bio coming soon.</p>:<><p className="team-role">{x.role}</p><p>{x.bio}</p></>}</div></article>)}</div></section></>}
export function Contact(){return <><Seo page="contact" path="/contact"/>
  <Hero className="contact-page-hero" image={images.exterior} title="How can we help?" text="Choose the clearest next step for your family, or contact the Garden Street team directly."/>
  <section className="contact-hub"><div className="contact-heading"><Section eyebrow="Get in touch" title="Start with what you need."/><p>Questions about classes, registration, parties or studio rentals are welcome. Choose an option below so you can get where you need to go quickly.</p></div><div className="contact-choice-grid">
    <article className="contact-choice contact-choice-primary"><span className="contact-choice-number" aria-hidden="true">01</span><p className="eyebrow">Questions & inquiries</p><h3>Email the studio</h3><p>Send a general question or ask about a party, rental or program.</p><a className="contact-detail-link" href={`mailto:${siteSettings.email}`}>{siteSettings.email}</a><a className="button" href={`mailto:${siteSettings.email}`}>Open your email app</a><small>The address stays visible so you can also copy it into any email service.</small></article>
    <article className="contact-choice"><span className="contact-choice-number" aria-hidden="true">02</span><p className="eyebrow">Prefer to talk?</p><h3>Call the studio</h3><p>Call Garden Street directly with your question.</p><a className="contact-detail-link" href={`tel:${siteSettings.phone.replaceAll('-','')}`}>{siteSettings.phone}</a><a className="button ghost" href={`tel:${siteSettings.phone.replaceAll('-','')}`}>Call now</a></article>
    <article className="contact-choice"><span className="contact-choice-number" aria-hidden="true">03</span><p className="eyebrow">Ready to explore?</p><h3>Browse registration</h3><p>View current classes and availability in Studio Pro, the studio’s registration system.</p><ExternalCta className="button ghost" href={externalLinks.generalRegistration}>View classes & registration</ExternalCta></article>
  </div></section>
